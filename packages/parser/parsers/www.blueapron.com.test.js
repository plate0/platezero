...
const RecipeParser = require('./www.blueapron.com')
const { testAsset } = require('../test')

describe('www.blueapron.com', () => {
  ...
  describe('mexican chicken zucchini rice', () => {
    ...
    test('get ingredient lists', () => {
      ...
      expect(result.ingredient_lists[0].lines).toEqual([
        ...
        {
          name: 'Mexican Spice Blend (Ancho Chile Powder, Smoked Paprika, Garlic Powder, Ground Cumin & Dried Mexican Oregano)',
          quantity_numerator: 1,
          quantity_denominator: 1,
          preparation: undefined,
          optional: false,
    ...
    test('get procedure lists', () => {
      ...
      expect(result.procedure_lists[0].lines[0]).toEqual({
        ...
        title: 'Cook the rice:',
        text: 'In a medium pot, combine the **rice**,** a pinch of salt**, and **1 cup of water**. Heat to boiling on high. Once boiling, reduce the heat to low. Cover and cook, without stirring, 12 to 14 minutes, or until the water has been absorbed and the rice is tender. Turn off the heat and fluff with a fork. Cover to keep warm.',
      })
    })
  })
