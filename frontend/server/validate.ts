...
export const validateNewRecipe = validator({
  ...
  ingredient_lists: Joi.array().items({
    name: Joi.string(),
  ...
  procedure_lists: Joi.array().items({
    name: Joi.string(),
    steps: Joi.array().required().items(Joi.string().required())
  }).required()
})

export const validateNewUser = validator({
 username: Joi.string().regex(/[a-zA-Z][a-zA-Z0-9\-_]+/).min(2).max(25).required(),
